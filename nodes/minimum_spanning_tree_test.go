...
func TestMinimumSpanningTree(t *testing.T) {
	...
	if len(got.Nodes) != 5 {
		t.Errorf("tree has %d nodes, want 5", len(got.Nodes))
	}
	// A spanning tree of 5 vertices has exactly 4 edges.
	if len(got.Edges) != 4 {
		t.Errorf("tree has %d edges, want 4: %+v", len(got.Edges), got.Edges)
	}
	if got.Directed {
	...
	if sum != 8 {
		t.Errorf("emitted tree edge weights sum to %v, want 8", sum)
	}
}
...
func TestMinimumSpanningTreeAgainstBruteForceOracle(t *testing.T) {
	...
	if sum != want {
		t.Errorf("tree weight = %v, exhaustive oracle = %v", sum, want)
	}
	// Describe is the documented way to weigh the emitted tree.
	stats, err := nodes.Describe(ctx, ax, got)
	if err != nil || stats.Error != "" {
		t.Fatalf("Describe: err=%v nodeErr=%s", err, stats.Error)
	}
	if stats.TotalWeight != want {
		t.Errorf("Describe total_weight = %v, exhaustive oracle = %v", stats.TotalWeight, want)
	}
}
...
func TestMinimumSpanningTreeComposes(t *testing.T) {
	...
	cyc, err := nodes.DetectCycle(ctx, ax, mst)
...
func TestMinimumSpanningTreeForest(t *testing.T) {
	...
	if len(got.Edges) != 2 {
		t.Errorf("forest has %d edges, want 2", len(got.Edges))
	}
	stats, err := nodes.Describe(ctx, ax, got)
	if err != nil || stats.Error != "" {
		t.Fatalf("Describe: err=%v nodeErr=%s", err, stats.Error)
	...
	if stats.ComponentCount != 2 {
		t.Errorf("component_count = %d, want 2", stats.ComponentCount)
	}
	if stats.TotalWeight != 5 {
		t.Errorf("total_weight = %v, want 5", stats.TotalWeight)
	}
}

func TestMinimumSpanningTreeRejectsDirected(t *testing.T) {
	ctx, ax := context.Background(), newTestContext(t)
	_, err := nodes.MinimumSpanningTree(ctx, ax, dagFixture())
	if err == nil {
		t.Errorf("expected directed input to be rejected")
	}
}
...
func TestMinimumSpanningTreeNilGraph(t *testing.T) {
	ctx, ax := context.Background(), newTestContext(t)
	_, err := nodes.MinimumSpanningTree(ctx, ax, nil)
	if err == nil {
		t.Errorf("expected an error for a nil graph")
	}
}
